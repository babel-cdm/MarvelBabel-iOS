...
open class BaseInteractor<Input, Output, Process>: Disposable {
    ...
    @objc private func disposeBagValueDisposed(_ notification: Notification) {
        if let userInfo = notification.userInfo as? [String: Any],
           let bag = userInfo[disposeBagNotificationParameter] as? DisposeBag {
            if bag === disposeBag {
                dispose()
            }
        }
