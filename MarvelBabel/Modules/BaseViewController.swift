...
private protocol BaseViewDependencies: class {
 ...
 */
open class BaseViewController<Presenter>: UIViewController,
    ...
    override open func didMove(toParent parent: UIViewController?) {
        ...
        if parent == nil {
            presenter.onNavBarLeftButtonClicked(false)
        }
    }
    ...
    public func showLoading() {
        if loadingView == nil {
            ...
            activityIndicatorView.color = UIColor(named: "App-Accent")
            activityIndicatorView.center = view.center
            activityIndicatorView.startAnimating()
            
            loadingView.addSubview(childView: activityIndicatorView, constraintType: .container)
            view.addSubview(childView: loadingView, constraintType: .center)
     ...
     */
    public func showNativeAlertView(title: String,
                                    message: String,
    ...
    private func setupNavBar() {
        setNavBarTitleAttributes(color: UIColor(named: "App-Primary"), font: UIFont.systemFont(ofSize: 20))
        setNavBarButtons()
        
        navigationController?.navigationBar.tintColor = UIColor(named: "App-Accent")
        navigationController?.navigationBar.barTintColor = UIColor(named: "App-Secondary")
    }
    ...
    private func setupNavBarTitleImage(_ image: UIImage, width: CGFloat = 150) {
        ...
        let titleView = UIView(frame: CGRect(x: .zero, y: .zero, width: width, height: 44))
    ...
    private func getNavBarViewControllerCount() -> Int {
        return navigationController?.viewControllers.count ?? .zero
    }
    ...
    @objc private func onNavBarLeftButtonClicked(_ sender: UIBarButtonItem) {
        presenter.onNavBarLeftButtonClicked(true)
    }
    ...
    private func createBarButtonItem(image: String,
        ...
        btnItem.imageEdgeInsets = UIEdgeInsets(top: 7, left: isLeftButton ? .zero : 7, bottom: 7, right: 7)
    ...
    private func resizeBaseVerticalScrollViewIfNeeded() {
        if let scrollView = view as? UIScrollView,
           scrollView.contentLayoutGuide.layoutFrame.height < scrollView.safeAreaLayoutGuide.layoutFrame.height &&
            scrollView.contentLayoutGuide.layoutFrame.height > .zero {
            let contentView = scrollView.subviews.first ?? scrollView
            let scrollViewConstraints = ViewUtils.getContainerConstraints(item: contentView,
