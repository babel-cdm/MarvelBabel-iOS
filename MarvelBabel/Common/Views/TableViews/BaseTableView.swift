...
/**
 Base table view.
 */
open class BaseTableView: UITableView, UITableViewDataSource, UITableViewDelegate {
    ...
    private weak var tableViewDataSource: BaseTableViewDataSource?
    private weak var tableViewDelegate: BaseTableViewDelegate?
    ...
    /**
     ...
     The data source must adopt the `BaseTableViewDataSource` protocol. The data source is not retained.
     
     - Parameter dataSource: The object that acts as the data source of the table view.
     */
    public func setupDataSource(_ dataSource: BaseTableViewDataSource?) {
    ...
    /**
     ...
     The delegate must adopt the `BaseTableViewDataSource` protocol. The delegate is not retained.
     
     - Parameter delegate: The object that acts as the delegate of the table view.
     */
    public func setupDelegate(_ delegate: BaseTableViewDelegate?) {
        self.delegate = self
    ...
    open func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return tableViewDataSource?.numberOfRowsInSection(tableView.tag, section) ?? .zero
    }
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cellForRowAtConfigurator = cellForRowAtConfigurator {
            ...
            if let baseCell = cell as? BaseTableViewCell,
               let disclosureIndicatorColor = baseCell.disclosureIndicatorColor {
    ...
    open func tableView(_ tableView: UITableView, didHighlightRowAt indexPath: IndexPath) {
        ...
        /// Set highlight color
        if let baseCell = tableView.cellForRow(at: indexPath) as? BaseTableViewCell,
           let highlightColor = baseCell.highlightColor {
            let selectedBackgroundView = UIView()
            selectedBackgroundView.backgroundColor = highlightColor
            
            if baseCell.selectedBackgroundView != nil {
                baseCell.selectedBackgroundView?.addSubview(childView: selectedBackgroundView,
                                                            constraintType: .container)
            } else {
                baseCell.insertSubview(childView: selectedBackgroundView,
                                       at: 0,
                                       constraintType: .container)
            }
        }
    ...
    open func tableView(_ tableView: UITableView, didUnhighlightRowAt indexPath: IndexPath) {
        ...
        if let baseCell = tableView.cellForRow(at: indexPath) as? BaseTableViewCell,
           baseCell.highlightColor != nil {
            if baseCell.selectedBackgroundView != nil {
                baseCell.setSelected(baseCell.isSelected, animated: true)
            } else {
                baseCell.subviews.first?.removeFromSuperview()
            }
        }
    }
