...
extension UIView {
    ...
    /**
     ...
     - shadowOpacity: Transparency of the shadow. By default is 0.2.
     */
    public func setShadow(side: ViewConstants.Sides,
        ...
        switch side {
        case .top:
            layer.shadowOffset = CGSize(width: .zero, height: -(shadowOffset))
        case .bottom:
            layer.shadowOffset = CGSize(width: .zero, height: shadowOffset)
        case .right:
            layer.shadowOffset = CGSize(width: shadowOffset, height: .zero)
        case .left:
            layer.shadowOffset = CGSize(width: -(shadowOffset), height: .zero)
        }
    }
    ...
    public func addContainerConstraints(childView: UIView) {
        let constraints = ViewUtils.getContainerConstraints(item: childView, toItem: self)
    ...
    public func addCenterConstraints(childView: UIView) {
        let constraints = ViewUtils.getCenterConstraints(item: childView, toItem: self)
        addConstraints(constraints)
    }
...
extension UIScrollView {
    ...
    public func setScrollSizeOnKeyboard(notification: NSNotification) {
        guard let info = notification.userInfo,
            ...
            Logger.print("Error: No size for keyboard in info")
            return
        }
...
extension UIViewController {
    ...
    public func checkBaseVerticalScrollView() {
        if hasBaseVerticalScrollView {
            ...
            var scrollViewConstraints = ViewUtils.getContainerConstraints(item: contentView,
...
extension UINavigationController {
     ...
     */
    public func pushRootViewController(animated: Bool) {
     ...
     */
    public func pushModalViewController(_ view: UIViewController, animated: Bool) {
    ...
    /**
     ...
     - Parameter animated: Boolean who decides whether to animate the transition with 'modal' efect.
     */
    public func popModalViewController(toRootVc: Bool = false, animated: Bool) {
        var isAnimated = animated
        if animated {
            let transition = CATransition()
            transition.duration = 0.4
